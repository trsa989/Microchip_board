   ...
   const NetBuffer *buffer, size_t offset, uint8_t hopLimit)
{
   ...
   case ICMPV6_TYPE_ECHO_REPLY:
       //Process Echo Reply message
	   icmpv6ProcessEchoReply(interface, pseudoHeader, buffer, offset);
	   break;
...
void icmpv6ProcessEchoReply(NetInterface *interface, Ipv6PseudoHeader *requestPseudoHeader,
   const NetBuffer *request, size_t requestOffset)
{
   ...
   //Debug message
   TRACE_INFO("ICMPv6 Echo Reply message received (%" PRIuSIZE " bytes)...\r\n", requestLength);
   //Dump message contents for debugging purpose
