   ...
   uint_t flags)
{
   ...
   else
   {
     ...
     if(interface->ipv6Context.isRouter){
       //Forward the packet according to the routing table
       //Case: when received ICMP request from other interface and you do a ICMP reply
       //This is the only way to route correctly the answer
       //More clear to analyze Net Prefix and forward only if necessary
       error = ipv6ForwardPacket(interface, buffer, offset);
       return error;
     }
#endif   // IPV6_ROUTING_SUPPORT == ENABLED
