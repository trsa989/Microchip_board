   ...
   if(!found)
   {
      ...
      for(i = 1; i < IPV6_ADDR_LIST_SIZE; i++)
      {
         ...
         if(entry->state == IPV6_ADDR_STATE_INVALID)
         {
            ...
            //Add network prefix to Route list!           
            ipv6AddRoute(&option->prefix, option->prefixLength, interface, NULL, 1);
            
            //Create default route: UNIQUE Local addresses will be forwarded to the default router.
            ipv6AddRoute(&option->prefix, 16, interface, &option->prefix, 1);            
            #endif
